...
//     scheme: 'http',
//     host: 'localhost:8080',
//     headers: { 'X-huggingface-Api-Key': <YOUR_HUGGING_FACE_API_KEY> },  // Replace with your inference API key
// });
...
const client: WeaviateClient = weaviate.client({
  ...
  headers: { 'X-huggingface-Api-Key': process.env.HUGGING_FACE_API_KEY as string }, // Replace with your inference API key
});
...
async function runFullExample() {
  ...
  let with_vector_query = await client.graphql
    .get()
    .withClassName('JeopardyQuestion')
