...
async function importData() {
  ...
  let categories = ['Network', 'Printing', 'Hardware', 'Software'];
...
async function scheduleClassification() {
  let response = client.classifications
    .scheduler()
    .withType('zeroshot')
...
async function showData() {
  const response = await client.graphql
    ...
    .withClassName('Ticket')
    .withFields(
      'body category{ ... on Category{name}} _additional{classification{basedOn classifiedFields completed id scope}}'
    )
    .do();
