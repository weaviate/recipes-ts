...
async function showData() {
  const response = await client.graphql
    ...
    .withClassName('Comment')
    .withFields(
      'body training_data sentiment{ ... on Sentiment{name}} _additional{classification{basedOn classifiedFields completed id scope}}'
    )
    .do();
  let comments = response.data['Get']['Comment'];
  for (const obj of comments) {
    console.log('####');
    console.log('Comment:', obj['body']);
    console.log('Sentiment:', obj['sentiment']);
    console.log('Training Data?:', obj['training_data']);
  }
}
