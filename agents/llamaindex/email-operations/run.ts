...
async function main() {
    ...
    const emailSender = async ({ text, subject }: { text: string, subject: string}) => {
        const response = await resend.emails.send({
            from: 'random@mydomain.com', // Replace with your default Resend email address
            to: 'delivered@resend.dev',  // Replace with the destination email address
            subject: subject,
            html: text,
        });
    ...
    // Step 3: Describe your tools for your Agent
    const emailTool = FunctionTool.from(emailSender, {
        name: 'emailSender',
        description: 'Use this tool to send emails, provide text in html format',
        parameters: {
            type: 'object',
            properties: {
    ...
    // Step 5: Enable event logging for your Agent
